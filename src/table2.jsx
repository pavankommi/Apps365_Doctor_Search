        ...
        return (
            <ScrollView>
                <TextInput
                    ...
                    autoFocus={true}
                />
                {filterData.length == 0 ? <Text>No Result</Text> : <View>
                    ...
                    <Table borderStyle={{ borderWidth: 1, borderColor: '#c8e1ff' }}>
                        {filterData?.map(e => e?.map(f => (
                            // <Text>{JSON.stringify(Object.keys(f[0]))}</Text>
                            <Row
                                data={filterData.length == 0 ? [] : Object.keys(f[0] || {})}
                            />
                        )))}
                        {filterData.map(e => e.map(f => f.map(g => (
